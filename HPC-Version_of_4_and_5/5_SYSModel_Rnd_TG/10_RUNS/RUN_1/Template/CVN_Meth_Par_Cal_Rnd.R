...
CVN_Meth_Par_Cal_Rnd<-function(TG,RNA_GI,Mutation_Info,GI,Hyper_GI,TG_Par_Info){
  RNA_TG<-RNA[TG,]
  ## CNV data 
  CNV_Gen_List=rownames(CNV_Methyl$CNV)
  if(TG%in%CNV_Gen_List){  ### If TG has CNV data
    CNV_TG<-CNV_Methyl$CNV[TG,] ## Trasform CNV 
  ...
  if (TG%in%Methyl_Gen_List){  ### If TG has Methylation data
  ...
  ###################################
  ## Parent Calculation for Random Target Genes
  source("Parents_Calc.R")
  PG<-Parents_Calc(GI,Hyper_GI,TG_Par_Info)
  ...
  ############################# Calculates the ratio of Non-syn wild type and after that we need to create the same ratio of group 1 vs. group 2
  ...
  Final_Data<-list(
    ...
    All_Mut_Indx=nrow(Mutation_Info)
  )
  return(Final_Data)
}