## Lat Update 6/9/2020
Finding_Outliers_Using_Lognorm_WI_NewFilter<-function(){
  ...
  print(nbaplot_All)
  ##----------------- Save as a Figure-------------------##
  file_Plot=paste0("./Results/All_Genes_With_Thresholds_NewFilter_",CANCER_TYPE,".pdf")
  text.factor<-3
  dpi <- text.factor * 100
  width.calc <- 2000 / dpi
  ...
  write.table(Filter_Summ_Rl, "./Results/Filter_Summ_Rl_NewFilter.txt", sep="\t",row.names = FALSE)
  
  ############ save the Candidate Genes in Excel sheet
  Summryfiles_Cand_exl_all <-paste0("./Results/Filter_Summ_Rl_NewFilter.xlsx")
  ...
  Summryfiles_Cand <-paste0("./Results/Candidate_GenesInfo_NewFilter_",CANCER_TYPE,".txt")
  write.table(Candidate_GenesInfo, Summryfiles_Cand, sep="\t",row.names = FALSE)
  
  ############ save the Candidate Genes in Excel sheet
  Summryfiles_Cand_exl <-paste0("./Results/Candidate_GenesInfo_NewFilter_",CANCER_TYPE,".xlsx")
  ...
  file_Plot=paste0("./Results/Labeled_Candidate_Genes_AllMutRate_NewFilter_",CANCER_TYPE,".pdf")
  text.factor<-2
  dpi <- text.factor * 100
  width.calc <- 2300 / dpi
  height.calc <- 1500 / dpi
  ...
  file_Plot=paste0("./Results/Labeled_Candidate_Genes_5Percent_NewFilter_",CANCER_TYPE,".pdf")
  text.factor<-2
  dpi <- text.factor * 100
  width.calc <- 2300 / dpi
  height.calc <- 1500 / dpi
