...
if(length(All_GIs)>0){
  ...
  for(GIIdx in 1:length(All_GIs)){ ####
    ...
    Saved_File_name=paste0("./RUNS/RUN_",Params$Iteration,"/Results/Res_NonSyn_Wild_",GIIdx,"_",GI,".RData")
    save(RES,file=Saved_File_name)
  }
  
  #####################
  source("Calc_Results_Measure.R")
  Calc_Results_Measure(Params$Iteration)
  
  setpath=dirname(dirname(dirname(getwd())))
  setwd(setpath)
  
  source("Analysis_Impacts_New.R")
  Analysis_Impacts_New(Params$Iteration)
