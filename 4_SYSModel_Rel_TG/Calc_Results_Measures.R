Calc_Results_Measures<-function(CC){
  setpath="./Results"
  setwd(setpath)
        ...
        for(i in 1:length(Current_RES)){
          ...
          if(length(Curr_GI)){
            for(J in 1:length(Curr_GI)){
              ...
              Data_Analysis=do.call(cbind.data.frame, Curr_Conection[1:3])
              Parent_Genes=length(unlist(Curr_Conection[4]))
              Data_Analysis=cbind(Data_Analysis,as.data.frame(Parent_Genes))
              samples_Wild_NonSyn=do.call(cbind.data.frame, Curr_Conection[7:14])
              Data_Analysis=cbind(Data_Analysis,samples_Wild_NonSyn)
              if (is.na(Curr_Conection[[5]])){
                Measures=list()
                Measures$Bht_Nm_M=0
                Measures$WPval=0
                ...
                source("../Bhatt_NonSyn_Wild.R")
                Measures<-Bhatt_NonSyn_Wild(Curr_Conection[[5]],Curr_Conection[[6]])
              }
  ...
  Results_FileName=paste0("Res_NonSyn_Wild_",CANCER_TYPE,".txt")
  write.table(Results_Impacts,Results_FileName, sep="\t",row.names = FALSE)
  }